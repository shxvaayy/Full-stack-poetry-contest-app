...
export default function PastWinnersPage() {
  ...
  const WinnerCard = ({ position, icon: Icon, color, title }: { position: number, icon: any, color: string, title: string }) => {
    ...
    return (
      <div className="text-center transform hover:scale-105 transition-all duration-300 hover:shadow-lg p-4 rounded-xl">
        <div className={`w-16 h-16 ${color} rounded-full flex items-center justify-center mx-auto mb-4 transition-all duration-300 hover:scale-110 shadow-lg hover:shadow-xl`}>
          <Icon className="text-2xl text-white drop-shadow-[0_0_12px_rgba(255,255,255,0.8)]" size={24} />
        </div>
        <h4 className="font-semibold text-gray-900 mb-2 hover:text-gray-700 transition-colors duration-300">{title}</h4>
  ...
  return (
    ...
    >
      <div className="max-w-4xl mx-auto px-4">
        ...
        <Card className="text-center mb-12 transform hover:scale-105 transition-all duration-500 hover:shadow-2xl border-0 bg-gradient-to-br from-green-50 via-white to-blue-50 overflow-hidden relative group">
          ...
          <CardContent className="p-8 relative z-10">
            <h2 className="text-3xl font-bold bg-gradient-to-r from-green-600 to-blue-600 bg-clip-text text-transparent mb-6">{timeline.result_announcement_date ? new Date(timeline.result_announcement_date).getFullYear() : '____'} - Our Inaugural Year</h2>
            ...
            <div className="bg-gradient-to-br from-gray-50 to-green-50 rounded-lg p-6 border border-green-200 group-hover:border-green-300 transition-all duration-300">
        ...
        <Card className="transform hover:scale-105 transition-all duration-500 hover:shadow-2xl border-0 bg-gradient-to-br from-purple-50 via-white to-pink-50 overflow-hidden relative group">
          ...
          <CardContent className="p-8 relative z-10">
            <h3 className="text-2xl font-bold bg-gradient-to-r from-purple-600 to-pink-600 bg-clip-text text-transparent mb-6 text-center">Contest Timeline</h3>
            <div className="space-y-4">
              <div className="flex items-center transform hover:scale-105 transition-all duration-300 p-3 rounded-lg hover:bg-purple-50">
                <div className="w-4 h-4 bg-green-500 rounded-full mr-4 shadow-lg"></div>
                <div>
                  <p className="font-semibold text-gray-900 hover:text-green-700 transition-colors duration-300">{formatDateTime12h(timeline.contest_launch_date)}</p>
                  <p className="text-gray-600 hover:text-gray-700 transition-colors duration-300">Contest Launch & Submissions Open</p>
                </div>
              </div>
              <div className="flex items-center transform hover:scale-105 transition-all duration-300 p-3 rounded-lg hover:bg-yellow-50">
                <div className="w-4 h-4 bg-yellow-500 rounded-full mr-4 shadow-lg"></div>
                <div>
                  <p className="font-semibold text-gray-900 hover:text-yellow-700 transition-colors duration-300">{formatDateTime12h(timeline.submission_deadline)}</p>
                  <p className="text-gray-600 hover:text-gray-700 transition-colors duration-300">Submission Deadline</p>
                </div>
              </div>
              <div className="flex items-center transform hover:scale-105 transition-all duration-300 p-3 rounded-lg hover:bg-blue-50">
                <div className="w-4 h-4 bg-blue-500 rounded-full mr-4 shadow-lg"></div>
                <div>
                  <p className="font-semibold text-gray-900 hover:text-blue-700 transition-colors duration-300">{formatDateTime12h(timeline.result_announcement_date)}</p>
                  <p className="text-gray-600 hover:text-gray-700 transition-colors duration-300">Results Announcement</p>
                </div>
              </div>
            </div>
          </CardContent>
